...
use crate::components::story_list::{ Comment, StoryItem, StoryPageData };
...
pub async fn get_story(id: i64) -> Result<StoryPageData, reqwest::Error> {
    ...
    let comment_futures = story.item.children.iter().map(|&id| get_comment(id));
    let comments = join_all(comment_futures).await
        .into_iter()
        .filter_map(|c| c.ok())
        .collect();
...
pub async fn get_comment_with_depth(id: i64, depth: i64) -> Result<Comment, reqwest::Error> {
    ...
    if depth > 0 {
        let sub_comments_futures = comment.children
            .iter()
            .map(|story_id| get_comment_with_depth(*story_id, depth - 1));
        comment.sub_comments = join_all(sub_comments_futures).await
            .into_iter()
            .filter_map(|c| c.ok())
            .collect();
    }
